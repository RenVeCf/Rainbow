...
import com.ipd.rainbow.ui.activity.classroom.OwnedClassRoomActivity
import rx.Observable

class MineClassRoomFragment : ListFragment<BaseResult<List<ClassRoomBean>>, ClassRoomBean>() {
    companion object {
    ...
    override fun loadListData(): Observable<BaseResult<List<ClassRoomBean>>> {
        return ApiManager.getService().classroomBuyedList(GlobalParam.getUserIdOrJump(), Constant.PAGE_SIZE, page)
    }
    ...
    override fun setOrNotifyAdapter() {
        if (mAdapter == null) {
            mAdapter = ClassRoomAdapter(mActivity, data, true, {
                //itemClick
                if (it.IS_BUY == 1) {
                    //已购买
                    OwnedClassRoomActivity.launch(mActivity, it.ORDER_NO)
                } else {
