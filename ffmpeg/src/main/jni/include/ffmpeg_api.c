...
Java_com_ipd_ffmpeg_FFmpegApi_exec(JNIEnv *env, jclass jclass,jobjectArray commands){
    int cmdnum = (*env)->GetArrayLength(env, commands);
    (*env)->GetJavaVM(env, &jvm);
    m_clazz = (*env)->NewGlobalRef(env, jclass);
    //---------------------------------C语言 反射Java 相关----------------------------------------
